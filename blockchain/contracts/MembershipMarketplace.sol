 ...
 * @notice Facilitates primary sales, renewals, and secondary listings for PushCampus memberships.
...
contract MembershipMarketplace is AccessControl, Pausable, ReentrancyGuard {
        ...
        whenNotPaused
    {
        ...
        uint256 price = courseInfo.priceWei;
        ...
        if (payout > 0) {
            ISplitPayout(courseInfo.splitter).recordPayment{value: payout}(msg.sender);
        }
    ...
    function createListing(uint256 courseId, uint256 priceWei, uint64 durationSeconds)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        emit ListingCreated(courseId, msg.sender, priceWei, listing.listedAt, expiresAtTs, expiresAt);
    }
        ...
        whenNotPaused
    {
        ...
        if (sellerProceeds > 0) {
            _transferValue(seller, sellerProceeds);
        }
    ...
    function renew(uint256 courseId, uint256 maxPrice) external payable nonReentrant whenNotPaused {
        MembershipPass1155.Course memory courseInfo = membership.getCourse(courseId);
        if (membership.balanceOf(msg.sender, courseId) == 0) {
            revert PassNotHeld();
        }
    ...
    function _collectPayment(uint256 requiredAmount) internal {
        if (requiredAmount == 0) {
            if (msg.value > 0) {
                _transferValue(msg.sender, msg.value);
            }
            return;
        }
        if (msg.value < requiredAmount) revert InsufficientPayment();
        uint256 surplus = msg.value - requiredAmount;
        if (surplus > 0) {
            _transferValue(msg.sender, surplus);
        }
    }
