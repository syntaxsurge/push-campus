...
export function GroupSettingsForm({ group }: GroupSettingsFormProps) {
  ...
  }, [membershipAddress, publicClient, pushChainClient])
  ...
  }, [publicClient, pushChainClient, registrarAddress])
  ...
  const handleRegisterCourse = useCallback(async () => {
    ...
    if (!pushChainClient || !registrarService) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
        ...
        {billingCadence === 'monthly' && (
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Monthly price ({NATIVE_TOKEN_SYMBOL})</FormLabel>
                <FormControl>
                  <Input
                    type='number'
                    min='0'
